...
async function adjustPerifpherySourceCode(address) {
  ...
  const contract = await ethers.getContractAt('AstroFactory', address)
...
async function main() {
  ...
  AstroFactory = await deploy('AstroFactory', [admins._devaddr])

  await adjustPerifpherySourceCode(AstroFactory)
}
